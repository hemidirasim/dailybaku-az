...
async function deleteEnglishDemoArticles() {
  try {
    console.log('Searching for English demo articles to delete...\n');
    ...
    for (const article of allArticles) {
      ...
      // Check if English title contains "New" and "Political news"
      ...
      // Also delete "Azerbaijan national team achieves new success"
      if (enTranslation && enTranslation.title.includes('Azerbaijan national team achieves new success')) {
        if (!deletedIds.has(article.id)) {
          await prisma.article.update({
            where: { id: article.id },
            data: {
